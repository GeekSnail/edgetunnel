...
const START_PAGE = 2;
const DEPLOYMENTS_PER_PAGE = 2;
const PAGINATION_BATCH_SIZE = 1; //3
  ...
  while (true) {
    ...
    try {
      result = await backOff(() => listDeploymentsPerPage(page), {
        ...
        retry: (_, attempt) => {
          console.warn(`Failed to list deployments on page ${page}... retrying (${attempt}/${MAX_ATTEMPTS})`);
          return true;
        },
      });
    } catch (err) {
...
async function deleteBatch(deploymentIds) {
  if (START_PAGE == 1) {
    ...
    console.log(`Found live production deployment to exclude from deletion: ${productionDeploymentId}`);
  }

  for (id of deploymentIds) {
    if (START_PAGE == 1 && id === productionDeploymentId) {
...
async function main() {
  ...
  if (!CF_PAGES_PROJECT_NAME) {
    throw new Error("Please set CF_PAGES_PROJECT_NAME as an env variable to your Pages project name");
  }
